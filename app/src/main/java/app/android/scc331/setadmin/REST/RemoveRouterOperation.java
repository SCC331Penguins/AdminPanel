...
public class RemoveRouterOperation extends RestOperation {

    private final String TAG = "REMOVERouter";

    public RemoveRouterOperation(Context context) {
        super(context);
    }

    public boolean start(String router_id){
        boolean object = false;
        try {
            new PostTask(router_id).execute();
        }catch (Exception e){
            e.printStackTrace();
        }
        return object;
    ...
    public class PostTask extends AsyncTask<String, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean aBoolean) {
            ...
            if (result){
                Toast.makeText(context,"Router removed successfully",Toast.LENGTH_LONG).show();
            }else{
                Toast.makeText(context, "Error removed router", Toast.LENGTH_LONG).show();
            }
    ...
    public boolean performPost(String router_id) throws IOException, JSONException {
        ...
        HttpPost post = new HttpPost(RestPaths.REMOVE_ROUTER);
        Log.d(TAG, RestPaths.REMOVE_ROUTER);
